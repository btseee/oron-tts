...
def test_the_model_card_states_every_fact_the_other_docs_defer_to_it():
    ...
    This is the long-form card, not the published one -- the published card is
    generated by scripts/model_card.py and links here. Its own copy of these six
    is guarded by test_model_card.py; both files need them, for different
    readers.
    """
...
def test_the_model_card_leaves_the_unmeasured_numbers_blank():
    """The blanks are the point, and they still are now that this file is the
    long-form companion rather than the published card.

    The published card links here, so a reader reaches this page; a number
    invented to fill a blank would mislead them exactly as it would have on the
    Hub. The blanks stay until the reporting run produces the figures.
    """
    card = (ROOT / "docs" / "model-card.md").read_text(encoding="utf-8")
