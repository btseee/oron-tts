# Model card — long form, for `btsee/oron-tts`

**This is not the file that ships.** The published card is generated by
`scripts/model_card.py`, which reads every number from `eval.json` and
`consistency.json` so it cannot drift from the run; it is deliberately short and
usage-first, carries all six of this document's disclosures in one line each,
and links back here. This page is the long version: the method, the per-source
figures, and the caveats that do not fit on a page someone reads before their
first synthesis.

Fill the blanks marked `<>` from the reporting run. Everything not marked is
already measured and should not change.
